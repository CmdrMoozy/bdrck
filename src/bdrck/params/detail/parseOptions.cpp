...
boost::optional<std::string> extractValue(std::string &parameter)
{
	auto idx = parameter.find('=');
	if(idx == std::string::npos)
		return boost::none;

	boost::optional<std::string> value(parameter.substr(idx + 1));
                                              ...
                                              Command const &command)
{
	...
	while(!parameters.parameters.empty())
	{
		...
		boost::optional<Parameter> parameter;
		try
		{
