...
{
	std::experimental::optional<bdrck::json::JsonValue> root;
	std::experimental::optional<bdrck::json::StringType> key;
	std::stack<bdrck::json::JsonValue *> current;

	ParseAllContext()
	        : root(std::experimental::nullopt),
	          key(std::experimental::nullopt),
	          current()
	{
	}
};
...
public:
	...
	bdrck::json::JsonValue *operator()(bdrck::json::MapType &map)
	{
		...
		context.key = std::experimental::nullopt;
		return ptr;
	}
...
std::experimental::optional<JsonValue> parseAll(std::istream &in)
{
	ParseAllContext context;
